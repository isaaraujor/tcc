...
try {
    $stmt = $con->prepare(" SELECT c.id_controle AS id_controle, c.data_cont AS data_controle, dt.id_disc AS id_disciplina, dt.id_turma AS id_turma, a.id_alunos AS id_aluno, f.qtde_faltas AS faltas
        FROM falta f INNER JOIN controle c ON f.controle_id = c.id_controle INNER JOIN alunos a ON f.aluno_id = a.id_alunos INNER JOIN disc_turma dt ON c.id_discTurma = dt.id_discTurma
        WHERE f.id_falta = ?",);
